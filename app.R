                ...
                conditionalPanel(
                  ...
                  selectInput("regional_contrib_correction",
                             ...
                             choices = c("FDR (Benjamini-Hochberg)" = "fdr",
                                       ...
                                       "None" = "none"),
                             selected = "fdr"),
                  uiOutput("regional_correction_recommendation")
                ),
                ...
                conditionalPanel(
                  ...
                  selectInput("hypothesis_correction",
                             ...
                             choices = c("FDR (Benjamini-Hochberg)" = "fdr",
                                       ...
                                       "None" = "none"),
                             selected = "fdr"),
                  uiOutput("hypothesis_correction_recommendation")
                ),
                ...
                conditionalPanel(
                  ...
                  selectInput("discovery_correction_method",
                             ...
                             choices = c("FDR (Benjamini-Hochberg)" = "fdr",
                                       ...
                                       "None" = "none"),
                             selected = "fdr"),
                  uiOutput("discovery_correction_recommendation"),
                  # Estimation display
                  tags$hr(style = "margin: 10px 0;"),
                ...
                tabPanel("7a. ROI-Level Permutation Testing",
                  ...
                  selectInput("roi_correction_method",
                             ...
                             choices = c("FDR (Benjamini-Hochberg)" = "fdr",
                                       ...
                                       "None" = "none"),
                             selected = "fdr"),
                  uiOutput("roi_correction_recommendation"),
...
server <- function(input, output, session) {
  ...
  # ========== MULTIPLE COMPARISON CORRECTION RECOMMENDATION HELPER ==========
  # Literature-based recommendations (McDonald 2014, Noble 2009, Benjamini & Hochberg 1995)
  get_correction_recommendation <- function(n_tests, analysis_type = "exploratory") {

    if (n_tests == 1) {
      return(list(
        recommended = "none",
        message = "Single test - no multiple comparison correction needed.",
        citation = "No correction required for single comparisons."
      ))
    }
    ...
    if (n_tests <= 10) {
      return(list(
        recommended = "fdr",
        message = paste0("Testing ", n_tests, " comparisons. FDR recommended to balance ",
                         "power and false positive control. Holm or Bonferroni acceptable for strict FWER control."),
        citation = "McDonald (2014): Bonferroni 'mainly useful when there are a fairly small number of multiple comparisons'"
      ))
    }
    ...
    if (n_tests <= 50) {
      return(list(
        recommended = "fdr",
        message = paste0("Testing ", n_tests, " comparisons. FDR recommended - ",
                         "Bonferroni becomes overly conservative with this many tests."),
        citation = "Noble (2009): 'Use of traditional Bonferroni is too conservative'"
      ))
    }
    ...
    return(list(
      recommended = "fdr",
      message = paste0("Testing ", n_tests, " comparisons. FDR strongly recommended - ",
                       "Bonferroni would result in excessive false negatives."),
      citation = "Benjamini & Hochberg (1995): FDR 'designed for large-scale testing'"
    ))
  }
 ...
 observe({
  ...
  # ========== MULTIPLE COMPARISON CORRECTION RECOMMENDATION OUTPUTS ==========

  # Regional (subregion/collective) correction recommendation
  output$regional_correction_recommendation <- renderUI({
    req(input$regional_contrib_level %in% c("subregion", "collective"))

    # Count tests based on level
    n_tests <- if(input$regional_contrib_level == "subregion") {
      # Count subregions
      regions <- unlist(ui_state$brain_areas)
      length(regions)
    } else {
      # Count brain areas
      length(ui_state$brain_areas)
    }

    if(is.null(n_tests) || n_tests == 0) n_tests <- 20  # Default estimate

    rec <- get_correction_recommendation(n_tests, "exploratory")

    # Auto-update the selection
    updateSelectInput(session, "regional_contrib_correction", selected = rec$recommended)

    tags$div(
      style = "background-color: #e7f3ff; padding: 8px; border-radius: 4px; margin-top: 5px;",
      tags$small(
        tags$strong("Recommendation: ", style = "color: #004085;"), rec$message,
        tags$br(),
        tags$em(rec$citation, style = "color: #666; font-size: 0.9em;")
      )
    )
  })

  # Hypothesis testing correction recommendation
  output$hypothesis_correction_recommendation <- renderUI({
    req(input$regional_contrib_level == "hypothesis")

    # Count hypothesis combinations
    combos <- hypothesis_combinations()
    n_tests <- if(!is.null(combos)) length(combos) else 0

    if(n_tests == 0) {
      return(tags$div(
        style = "color: #666; font-size: 0.85em; margin-top: 5px;",
        "Add combinations to see correction recommendation."
      ))
    }

    rec <- get_correction_recommendation(n_tests, "confirmatory")

    # Auto-update the selection
    updateSelectInput(session, "hypothesis_correction", selected = rec$recommended)

    tags$div(
      style = "background-color: #e8f4f8; padding: 8px; border-radius: 4px; margin-top: 5px;",
      tags$small(
        tags$strong("Recommendation: ", style = "color: #0c5460;"), rec$message,
        tags$br(),
        tags$em(rec$citation, style = "color: #666; font-size: 0.9em;")
      )
    )
  ...
  # Discovery (brute force) correction recommendation
  output$discovery_correction_recommendation <- renderUI({
    req(input$regional_contrib_level == "artificial")

    max_size <- input$discovery_max_combo_size
    if(is.null(max_size)) max_size <- 4

    # Estimate number of regions
    n_regions <- 20
    methods <- selected_methods()
    for(method in methods) {
      if(!is.null(analysis_results$correlation_methods_raw[[method]])) {
    ...
    # Calculate total combinations
    n_tests <- sum(sapply(1:max_size, function(k) choose(n_regions, k)))
  ...
  # ROI-level correction recommendation
  output$roi_correction_recommendation <- renderUI({
    # Count ROIs from available data
    n_rois <- 20  # Default

    # Try to get actual count from data
    if(!is.null(ui_state$processed_data)) {
      # Count numeric columns (excluding group)
      numeric_cols <- sapply(ui_state$processed_data, is.numeric)
      n_rois <- sum(numeric_cols)
    }

    rec <- get_correction_recommendation(n_rois, "exploratory")
  ...
  # Warning modal when user selects "None" correction for brute force with many tests
  observeEvent(input$discovery_correction_method, {
    req(input$regional_contrib_level == "artificial")
    req(input$discovery_correction_method == "none")
    ...
    if(n_tests > 10) {
      showModal(modalDialog(
        title = tags$span(icon("exclamation-triangle"), " Warning: No Multiple Comparison Correction"),
        tagList(
          tags$p(paste0("You are about to test ", format(n_tests, big.mark = ","),
                       " combinations without any correction for multiple comparisons.")),
          tags$p(tags$strong("Expected false positives at \u03B1=0.05: "),
                paste0("~", round(n_tests * 0.05), " spurious significant results")),
          tags$hr(),
          tags$p("The statistical literature strongly recommends FDR correction for exploratory analyses ",
                "with large numbers of tests (Noble 2009, Benjamini & Hochberg 1995).",
                style = "color: #666;"),
          tags$p("Without correction, you may report many false discoveries as significant findings.",
                style = "color: #856404; font-weight: bold;")
        ),
        footer = tagList(
          actionButton("confirm_no_correction", "Proceed Without Correction",
                       icon = icon("exclamation-triangle"),
                       style = "background-color: #dc3545; color: white;"),
          actionButton("use_fdr_instead", "Use FDR Instead (Recommended)",
                       icon = icon("check"),
  ...
  # Handle "Use FDR Instead" button
  observeEvent(input$use_fdr_instead, {
    updateSelectInput(session, "discovery_correction_method", selected = "fdr")
    removeModal()
  })

  # Handle "Proceed Without Correction" button
  observeEvent(input$confirm_no_correction, {
    removeModal()
    showNotification(
      "Proceeding without multiple comparison correction. Results should be interpreted with caution.",
      type = "warning",
      duration = 5
    )
  })
